...
- 파이썬은, 인터프리터 언어이기 때문에 Java 랑 똑같이 짜면 시간초과날 때가 있다. 이 부분은 무조건적으로 조심해야 한다. 인터프리터 언어의 한계라고 볼 수 있다.
- 그렇기 때문에, 파이썬은 가급적이면 코드 수를 압축적으로 줄여나가야 한다. 한방에 모두 실행할 수 있는 쪽으로 가야, 같은 완전탐색이라도 시간을 줄여나갈 수 있다. 
...
#### 코드에 대한 설명
        기본적으로 파이썬은, 이렇게 배열을 한방에 만들 수 있는 파워가 있다. 
        그것도 이중 for 문으로 만들어버릴 수 있다. 그리고 sum() 도 하나하나 할 필요가 없이, 배열 전체를 때려버리면 가능하다.
        특정 배열에 대한 합을 구하고, 연산할 때는, 배열 전체를 한방에 모두 만들 수 있다는 사실을 기억하고 활용하도록 하자. 같은 O(N^3) 인 것처럼 보여도, 충분히 가능하다.
