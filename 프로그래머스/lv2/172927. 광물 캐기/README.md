...
처음에 "최소값" 키워드가 나왔을 때 DP 혹은 Greedy 둘중 하나라고 생각한 것은 좋았다. 
이 때, Greedy 를 처리할 때, 묶어서 평균값을 만든 다음, 정렬을 혹시 하면 Greedy 조건을 만족할 수 있는지 파악하는 것도 좋은 생각인 것 같다. 
실제로 그렇게 했고, 이까지 걸리는 시간이 좀 걸렸다. 경험을 많이 하도록 하자.
