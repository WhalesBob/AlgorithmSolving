...
## 의식의 흐름
 ...
 2,3,4미터 거리의 지점에 좌석이 하나씩존재한다
 두명이 마주 보고 탄다고 할 때, 평형인 상태에서 각각에 의 해 시소에 걸리는 토크 상쇄
균형을 맞추면, 시소짝꿍이다. 두명이 타는거다.

탑승한 사람의 무게와, 시소 축과 좌석 간의 거리의 곱이 양쪽 다 같을 때.
어떻게든 맞추면된다.

근데 2,3,4. 즉, 1 : 1 / 1 : 1.5 / 1 : 2 셋중하나만 만족하면된다.
시소 짝꿍이 "몇 쌍" 존재하는지 구하여 return 한다.

완탐되나?

만약에, 맵으로 해서, 존재하면 체크, 아니면 뺀다.

맵으로 하면, 그냥 4n 이다. 완전탐색 충분히 가능하다.
