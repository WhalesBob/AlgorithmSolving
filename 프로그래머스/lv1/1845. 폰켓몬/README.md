...
N 개 중 N/2 마리를 선택한다. 
종류 번호가 담긴 배열 nums 가 주어지면, N/2 마리 폰켓몬 선택방법 중 가장 많은 종류를 선택하는 방법을 찾아, 종류 번호와 갯수를 return 하도록 만들자 
    
각 한마리씩 선택하는 방법이 제일 좋다.
각 한마리씩 못 고르면, 그냥 있는 숫자만큼 고르면 된다.
