...
- 파이썬에서 전역 변수를 선언할 때는 global a 와 같은 식으로 쓰며, 함수 내에서 다시 쓰고자 할 때는 다시 함수 내에서 global a 를 다시 선언해야 그 친구가 전역변수 인지 알고 사용할 수 있다.
- 팀을 두 개로 나누는 케이스에서도 DFS 를 통해 팀을 나누고, 끝까지 가서 계산하는 방식을 사용할 수 있다.
- 조합하는 방식의 알고리즘 과 비슷하게 흘러가고 있다. combination(array, visited, start, n,r) 과 DFS(index, size) 의 차이이다. visited, matrix 가 전역 변수로 들어갔으며, n 도 전역변수 취급이다. start, r 이 남아서 비슷하게 흘러간 것 같다. 결국 처음 생각했을 때의 조합론과 비슷하게 흘러갈 수 있다. 조합 의 변형이라고 생각해도 좋을 것 같다.
- 조합론 과 비슷한 방식으로 흘러갈 수 있는 문제는 이것 말고도 엄청 많다. 조합으로 해결할 수 있지 않을까? 하는 생각이 들면, 일단 조합 코드에서 어떻게 바꾸어야 잘 해결할 수 있을까 생각해 보자.
