...
public class Main {
    ...
    static void dfs(List<String> inputList, boolean[] visited, int currentIndex, int count) {
        if(count == 0) {
            getPuzzleCase(inputList, visited);
            return;
        }
    ...
    static void getPuzzleCase(List<String> inputList, boolean[] visited) {
        ...
        for (Integer[] indexArray : indexMatrix) {
            char[][] map = new char[3][3];
            setWordMap(map, inputList, indexArray);

            List<String> verticalWord = makeVerticalWord(map);
            ...
            if (verticalWord.size() == 0 && !alreadyHaveThisMap(map)) {
                mapList.add(map);
            }
        }
    }
    static boolean alreadyHaveThisMap(char[][] map) {
    ...
    static List<String> makeVerticalWord(char[][] map){
