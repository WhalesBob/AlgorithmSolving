...
- 월말 평가 문제였는데, 하나밖에 예제를 맞추지 못해 아쉽다.
- 이후에도, 엄청난 반례들이 많이 나왔다. 아직 내가 골드 3급 구현 문제에 익숙하지 못해서 그런가 보다.
- 앞으로는, 쉬운 문제가 있으면, 빠르고 정확하게 문제를 해결하여 시간을 더더 많이 아껴 보도록 해야 할 것이다.
- 그리고, 앞으로 0부터 무조건 숫자를 매기도록 하자. 이것때문에도 실수가 한번 나왔다.
- boolean 조건을 거는 함수에 대해서, 조금 더 꼼꼼하게 쳐다보도록 하자.
- 골드 3 급 문제를 더더 많이 풀자! 슬슬 조금만 더 하면, 골드 3의 벽도 허물 수 있을 것이다!
