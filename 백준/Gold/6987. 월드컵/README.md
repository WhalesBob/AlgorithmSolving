...
- 정말 백트래킹 어려운 문제를 풀어 보았다. 앞으로 백트래킹에 대해서 더더욱 익숙해져야 할 것이다.
  - 완탐 베이스로, 중간에 조건을 걸어주기만 해도 백트래킹이다. 
  - 3^15 를 다 보는 것이 아닌 것이다. 이번에는, 한 팀의 승/무/패 를 기준으로 다른 팀의 승/무/패를 매겨가면서 결과적으로 모두 0이 되었을 때 true를 만들었다.
  - 조건이 걸렸기 때문에, 백트래킹이 맞긴 한것이다.
  - 앞으로도, 완탐을 할 것이라고 생각하면 조금 단순하게 문제를 바라보자. 그리고, 조건을 걸 수 있는 방법이 있는지 알아보는 것도 좋은 방법일 것 같다.
  - 백트래킹... 더 풀어보자!
